...
cat(paste0("\n # Using ", threads, " thread(s) for processing BAM files.\n"))# Default to the number of available cores
if(length(grep("threads",args)) != 0) {
  threads = as.numeric(strsplit(args[grep("threads",args)], "=")[[1]][2])
  if(is.na(threads) || threads <= 0) {
    cat("\n\t # ERROR: Invalid number of threads specified. Please provide a positive integer.\n")
    quit()
  }
}

cat(paste0("\n # Using ", threads, " thread(s) for processing dyads.\n"))
...
process_dyad <- function(line) {
  ...
  if(file.size(paste0("comm",samp1,"_",samp2,"_SNPs")) > 0) {
    ...
    if(verbose == TRUE) {
      if(file.exists(paste0("wc -l comm",samp1,"_",samp2,"_SNPs2"))) {
        cat(paste0("\t\t###### Number of common SNPs after QC1: ", as.integer(strsplit(system(paste0("wc -l comm",samp1,"_",samp2,"_SNPs2"), intern=T)," ")[[1]][1])),"\n")
      } else {
      cat(paste0("\t\t###### Number of common SNPs after QC1: 0\n"))
      }
    }
    if(file.exists(paste0(samp1,"____",samp2,".tped")) == TRUE) {
    ...
    } else if(file.exists(paste0(samp1,"____",samp2,".tped")) == FALSE) {
...
results <- mclapply(1:nrow(combos), process_dyad, mc.cores = threads)

# Combine results and export
toExport <- do.call(rbind, results)
